...
(function() {
  ...
  PollingTask.prototype._getPrivate = function PollingTask_getPrivate(key) {
    return _PollingTask_private_[this.id][key];
  };

  /**
   * Determine if a task is running
   * @return {Boolean} true if the task is running.
   */
  PollingTask.prototype.isRunning = function PollingTask_isRunning() {
    return this._getPrivate('isRunning');
  };

  PollingTask.prototype.runTask = function PollingTask_runTask() {
    if (!this._getPrivate('isRunning')) {
      if (this.async) {
        this.runTaskAsync();
      } else {
        this.runTaskSync();
