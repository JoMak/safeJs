...
(function() {
  ...
  TypeDefinition.prototype.isValidWith = function TypeDefinition_isValidWith(value) {
    ...
    var isValid = this._types.some(function(type) {
      ...
      if (type.subDef && (type.subDef instanceof TypeDefinition)) {
        if (_.isArray(value)) {
        ...
        return (type.constructor && (value instanceof type.constructor));
      }
    ...
    'types': {
      ...
      set: function TypeDefinition_types_set(types) {
        ...
        types.forEach(function(type) {
          ...
          } else if (_.isString(type) && _.isEmpty(type)) {
            throw new TypeError('Strings in "type" property cannot be empty.');

          } else if (_.isObject(type) && !_.isFunction(type) && !_.isFunction(type.constructor)) {
            throw new TypeError('Prototype object: ' + type + ' in property "types" must have a constructor property that is of type "function".');

          } else if (!_.isObject(type) && !_.isString(type)) {
            throw new TypeError('object ' + type + ' in array "types" is an invalid type. It can be either: (1) An Object, (2) a string, (3) a function or (4) an array of 1, 2, or 3.');
          }
